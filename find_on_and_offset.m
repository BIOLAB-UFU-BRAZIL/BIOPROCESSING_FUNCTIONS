...
% Description...:
%                 find_on_and_offset finds and returns the indexes of on
%                 and offsets of bursts in a signal using Hilbert Transform
%                 and a reference interval within the signal
%                    
% Parameters....: 
%                 signal...-> input signal
...
%                                            interval [ms], default: 0
%                             'T1'........-> ending of the reference
%                                            interval [ms], default: 2000
%                             'f_low'.....-> low frequency of the filter
%                                            [Hz], default: 2
%                             'f_high'....-> high frequency of the filter
%                                            [Hz], default: sampFreq/2
%                             'f_cut'.....-> cutoff frequency for the
%                                            Hilbert filter [Hz], default:
%                                            8
%                             'order'.....-> order for the low and high
%                                            filters, default: 4
%                             'order_env'.-> order for the Hilbert filter,
%                                            default: 4
%                             'nSDth'.....-> number of standard deviations
%                                            to consider for comparison 
%                                            with the reference interval,
%                                            default: 2
%                             'thresh'....-> threshold to find the burst
%                                            moments, default: 0
%                             'min_len'...-> minimal burst length [ms],
%                                            default: 0
%                             'min_dist'..-> minimal distance between each
%                                            burst moment [ms], default:
%                                            2000
%
% Return........:
...
%% Default inputs
varname = {'T0' 'T1' 'f_low' 'f_high' 'f_cut' 'order' 'order_env' 'nSDth' 'thresh' 'min_len' 'min_dist'};
value_vect = [0 2000 2 ((sampFreq / 2) - 1) 8 4 4 2 0 0 2000];
...
nSDth = value_vect(8);
thresh = value_vect(9);
min_len = value_vect(10);
min_dist = value_vect(11);
...
tt = find(envFilt < th);
thVector = envFilt;
thVector(tt)=0; %zero all points bellow the threshold from the filtered envelope
thVector(x1:x2) = 0; %ignor any detection from the reference interval (from x1 to x2)

%localize onset and offset points with the given threshold
ind = thVector > thresh;
ind_start = [0 (diff(ind) > 0)] > 0;
ind_end = [0 (diff(ind) < 0)] > 0;
